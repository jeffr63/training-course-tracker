...
export default class CourseEdit {
  ...
  protected readonly paths = toSignal(
    this.#store.select(pathsFeature.selectPaths),
    { initialValue: [] },
  );
  protected readonly sources = toSignal(
    this.#store.select(sourcesFeature.selectSources),
    { initialValue: [] },
  );
  ...
  readonly #course = rxResource<Course, string>({
    ...
    stream: ({ params: id }) => {
      if (this.id() === "new")
        return of({ title: "", instructor: "", path: "", source: "" });
  ...
  save() {
    this.#store.dispatch(
      coursesActions.saveCourse({ course: this.#course.value() }),
    );
    this.#router.navigate(["/courses"]);
  }
}
